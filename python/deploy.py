...
def exchange_token(auth_token: str) -> str:
    response = requests.post("https://provision.convex.dev/api/authorize", json={
...
def stream_command(cmd: list[str], **kwargs):
    ...
    if handle.returncode != 0:
        raise Exception(f"Command {' '.join(cmd)} failed with return code {handle.returncode}")
...
def download_repo(body: dict):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for line in stream_command(['git', 'clone', repo_url, os.path.join(temp_dir, repo_name)]):
            yield { 'status': line }

        yield { 'status': "Installing dependencies..." }
        for line in stream_command([bun_path, 'install'], cwd=os.path.join(temp_dir, repo_name)):
            yield { 'status': line }
